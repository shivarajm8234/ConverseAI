...
export const calculateLeadScore = (sentiment: string, intent: string, importantInfo: string) => {
    let score = 0.5;
    if (sentiment === 'positive' || sentiment === 'POSITIVE') score += 0.2;
    if (intent.toLowerCase().includes('buy') || intent.toLowerCase().includes('rent')) score += 0.3;
    if (importantInfo.length > 50) score += 0.1;
    return Math.min(1.0, score);
};

export const updateLeadFromCall = async (customerId: string, analysis: any) => {
...
export const onboardNewCustomer = async (data: any) => {
