 ...
 */
function readAgiEnvironment(): Map<string, string> {
...
function isEagiEnhanced(agiEnv: Map<string, string>): boolean {
...
function shouldEndConversationAfterRecord(agiResultLine: string): boolean {
...
async function askKnowledgeGraph(text: string) {
...
async function askLLM(prompt: string, context: string) {
        ...
        try {
            ...
            if (text && text.trim().length > 1) {
                ...
                const reply = await askLLM(
                    text,
                    "Ather showroom information: Ather 450X price is Rs 1,45,000. Features include 105km range, 90kmph top speed.",
                );
...
async function transcribeSTT(wavFile: string) {
    try {
...
async function startAgent() {
    ...
    log(
        `agi_enhanced=${agiEnv.get('agi_enhanced') ?? 'unset'} → ${useEagiStream ? 'EAGI stream (fd 3)' : 'AGI RECORD FILE conversation (no early exit on fd 3)'}\n`,
    );
    ...
    try {
                ...
                try {
                    ...
                    if (bytesRead > 0) {
                        ...
                        if (maxAmp > SILENCE_THRESHOLD) {
                            if (!isSpeaking) log("🔊 Voice detected!\n");
                            isSpeaking = true;
                            silenceChunks = 0;
                        } else {
                            silenceChunks++;
        ...
        await processAudio().catch((e: unknown) =>
            log(`Stream Processor Error: ${e instanceof Error ? e.message : String(e)}\n`),
        );
    } catch (e: any) {
